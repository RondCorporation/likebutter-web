...
interface Props {
  task: Task & { actionType: 'BUTTER_TEST' };
  onClick: () => void;
}

export default function ButterTestTaskCard({
  task: initialTask,
  onClick,
}: Props) {
    ...
    try {
      ...
      if (data) {
        setTask((prevTask) => ({
          ...prevTask,
          status: data.status,
          details: data.details as ButterTestDetails,
        }));
      }
    } catch (err) {
  ...
  const renderImageContent = () => {
    if (task.status === 'COMPLETED' && result?.imageUrl) {
      return (
        <Image
          src={result.imageUrl}
