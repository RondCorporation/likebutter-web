...
import {
  UseDigitalGoodsReturn,
  DigitalGoodsFormData,
} from '../_hooks/useDigitalGoods';

interface DigitalGoodsDesktopViewProps {
  hook: UseDigitalGoodsReturn;
  formData: DigitalGoodsFormData;
}

export default function DigitalGoodsDesktopView({
  hook,
  formData,
}: DigitalGoodsDesktopViewProps) {
            ...
            <StudioButton
              text={
                isGenerating || isPolling
                  ? t('digitalGoods.generating')
                  : t('digitalGoods.generate')
              }
      ...
      >
        ...
        >
          <div className="flex flex-col items-center justify-center gap-2.5 p-2.5 absolute top-[15px] left-[15px] right-[15px] bottom-[15px] bg-studio-header rounded-[20px] border border-dashed border-studio-header">
            {(isGenerating || isPolling) && !resultImage ? (
              ...
              <div className="flex flex-col items-center justify-center gap-4 w-full h-full">
                <div className="flex flex-col items-center gap-2 text-center text-blue-400">
                  <div className="text-base font-pretendard-medium">
                    {t('digitalGoods.backgroundProcessing')}
                  </div>
                  <div className="text-sm font-pretendard max-w-[200px]">
                    {t('digitalGoods.takingLong')}
                  </div>
                  <button
                    onClick={() =>
